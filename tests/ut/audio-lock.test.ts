...
describe('Audio Lock Contention Tests', () => {
  test('rejects a second audio operation while a fade holds the lock', async () => {
    ...
    try {
      ...
      assert.strictEqual(await actor.emitAndWaitFor('changeMute', 'muteChanged', 1), 1);

      // Start from paused (robust against an externally running server)
      const state = await actor.emitAndWaitFor('getState', 'stateChanged');
      if (state === 1) {
        const paused = actor.waitFor('stateChanged', 8000);
        actor.socket.emit('changeState', 0);
        assert.strictEqual(await paused, 0);
      }

      // Play: the ~3s fade-in holds the audio lock
      const lockAcquired = observer.waitFor('lockChanged');
      ...
      const lockReleased = observer.waitFor('lockChanged', 8000);
      ...
      const playing = await actor.waitFor('stateChanged', 8000);
      assert.strictEqual(playing, 1);
      assert.strictEqual(await lockReleased, false);

      // After release the same operation succeeds
      assert.strictEqual(await actor.emitAndWaitFor('changeVolume', 'volumeChanged', 60), 60);

      // Cleanup: back to paused, unmuted
      const pausedAgain = actor.waitFor('stateChanged', 8000);
      actor.socket.emit('changeState', 0);
      assert.strictEqual(await pausedAgain, 0);
      assert.strictEqual(await actor.emitAndWaitFor('changeMute', 'muteChanged', 0), 0);
    } finally {
      actor.disconnect();
      observer.disconnect();
    }
