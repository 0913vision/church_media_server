...
| 속성 | 타입 | 접근 | 권한 | 설명 |
| --- | --- | --- | --- | --- |
| `playback` | `PlaybackState` | 읽기/쓰기 | any | Whether the deck is playing. Writing it fades in or out and holds the audio lock for the length of the fade. Refused with flowActive while a flow holds the gate: the run was handed the deck and puts it back itself, so the deck is not the panel's until it finishes. |
| `volume` | `number` (0–100) | 읽기/쓰기 | any | Output volume. Applies immediately, so it is safe to write continuously while dragging a fader. Refused with flowActive while a flow holds the gate: the run was handed the deck and puts it back itself, so the deck is not the panel's until it finishes. |
| `mute` | `MuteState` | 읽기/쓰기 | any | Whether output is muted. Refused with flowActive while a flow holds the gate: the run was handed the deck and puts it back itself, so the deck is not the panel's until it finishes. |
